function dni2(form) {
    ...
    if (isNaN(numdni) || numdni < 0 || numdni > 99999999 || numdni.toString().length === 0 || numdni.toString().length > 8) {
        alert("Este numero de DNI no es válido");
        return false;
    } else if (letra.length !== 1) {
        alert("Ingresa solo una letra");
        return false;
    } else if (!/^[A-Za-z]+$/.test(letra)) {
        alert("Solo se permiten letra");
        return false;
    } else {
        residuo = numdni % div;
